...
public class PeerInfo {
    ...
    // Reads data from a file, having each line be information about a peer and constructs list of PeerInfo objects from that data.
    public static List<PeerInfo> loadFromFile(String filename) throws IOException {

        List<PeerInfo> peers = new ArrayList<>();
        List<String> lines = Files.readAllLines(Paths.get(filename));

        // Goes through each line in the list, creating new PeerInfo object using values from parts array and adds to peers list
        for(String line : lines) {
            String[] parts = line.split(" ");
             peers.add(new PeerInfo(Integer.parseInt(parts[0]), parts[1], Integer.parseInt(parts[2]), parts[3].equals("1")));
        }
